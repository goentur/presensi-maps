        ...
        <form action="{{ isset($data)?route($attribute['link'].'update',$data->id):route($attribute['link'].'store') }}" method="post">
            ...
            <div class="row">
                ...
                <div class="col-lg-8 mb-3">
                    ...
                    <input required type="text" class="form-control @error('nama')is-invalid @enderror" value="{{ isset($data)?$data->user->name:old('nama') }}" id="nama" name="nama" placeholder="Masukan nama">
                    @error('nama')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                ...
                <div class="col-lg-6 mb-3">
                    ...
                    <input required type="email" class="form-control @error('email')is-invalid @enderror" value="{{ isset($data)?$data->user->email:old('email') }}" @isset($data) readonly @endif id="email" name="email" placeholder="Masukan email">
                    @error('email')
                    <span class="invalid-feedback" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                    ...
                    <select required name="hak_akses" id="hak_akses" class="form-control select2">
                        <option value="">Pilih salah satu</option>
                        <option @isset($data)@selected($data->user->roles->pluck('name')[0]=='admin')
                        @endisset value="admin">Admin</option>
                        <option @isset($data)@selected($data->user->roles->pluck('name')[0]=='pegawai')
                        @endisset value="pegawai">Pegawai</option>
                    </select>
                    @error('hak_akses')
                    <strong class="text-danger f-12">{{ $message }}</strong>
                    @enderror
